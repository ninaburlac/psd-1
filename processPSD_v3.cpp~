...
int main( int argc, char* argv[]){
  ...
  char *aoe_string;
  if ( dplms ) aoe_string = Form("AoE_dplms[%d]",chn);
  else aoe_string = Form("AoE[%d]",chn);
  cout << "Using AoE: " << aoe_string << endl;
  ...
  TH1D *AoE = new TH1D("AoE","; A/E; Counts",1000,0,0.2);
  //if ( dplms ) tier->Draw(Form("AoE_dplms[%d]>>AoE",chn),cut_e);
  //else tier->Draw(Form("AoE[%d]>>AoE",chn),cut_e);
  tier->Draw(Form("%s>>AoE",aoe_string),cut_e);
  c1->Update();
  ...
  tier->Draw(Form("%s/%g>>h_AoE",aoe_string, mean_aoe),cut_e);
  tier->Draw(Form("%s/%g>>h_AoE1",aoe_string, mean_aoe),cut_e1);
  tier->Draw(Form("%s/%g>>h_AoE2",aoe_string, mean_aoe),cut_e2);
  tier->Draw(Form("%s/%g>>h_AoE3",aoe_string, mean_aoe),cut_e3);
  ...
  tier->Draw(Form("%s/%g>>hAoE",aoe_string, mean_aoe),cut_e);
  tier->Draw(Form("%s/%g>>hAoE1",aoe_string, mean_aoe),cut_e1);
  tier->Draw(Form("%s/%g>>hAoE2",aoe_string, mean_aoe),cut_e2);
  tier->Draw(Form("%s/%g>>hAoE3",aoe_string, mean_aoe),cut_e3);
  ...
  //double totIntegral = hAoE->Integral(hAoE->GetXaxis()->FindBin(0.), hAoE->GetXaxis()->FindBin(1.1));
  double totIntegral = hAoE->Integral(1, hAoE->GetNbinsX());
  ...
  for ( int i = 0; i < nbinCut; i++ ){
    cutIntegral = hAoE->Integral(hAoE->GetXaxis()->FindBin(AoE_lowCut), hAoE->GetNbinsX());
  ...
  tier->Draw(Form("%s/%g>>h_sep", aoe_string, mean_aoe), cut_e_sep, "same");
  ...
  tier->Draw(Form("%s/%g>>h_2614", aoe_string, mean_aoe), cut_e_fep, "same");
  ...
  tier->Draw(Form("%s/%g >> h_bkg",aoe_string, mean_aoe), cut_e, "same");
