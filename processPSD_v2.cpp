...
int main( int argc, char* argv[]){
  ...
  //----------- A/E vs timestamp of calibrations------------- 
 
  const int nCal = 5;
  int start[nCal], end[nCal];
  double mu_AoE_st[nCal], mu_AoE_err_st[nCal], calib[nCal];
  start[0] = 1539616800, end[0] = 1539622080;
  start[1] = 1539936360, end[1] = 1539942120;
  start[2] = 1540476900, end[2] = 1540483500;
  start[3] = 1540974900, end[3] = 1540981380;
  start[4] = 1541686800, end[4] = 1541694300;
  double cc_min = 1000., cc_max = 1300.;

  TCanvas *ct = new TCanvas("ct","time");
  ct->Divide(2,3);

  for (int i = 0; i<nCal; i++){
    ct->cd(i+1);
    //gStyle->SetOptFit(111);
    TCut cut_time = Form("energy[%d]>%g && energy[%d]<%g && tempo[%d]>%d && tempo[%d]<%d", chn, cc_min, chn, cc_max, chn,start[i], chn, end[i]);
    double fMin_stab = 0.02;
    double fMax_stab = 0.07;
    TH1D *h_AoE_stab = new TH1D("h_AoE_stab","; A/E; Counts",100,fMin_stab,fMax_stab);
    tier->Draw(Form("AoE[%d]>>h_AoE_stab",chn),cut_time);
    ct->Update();
    
    TF1 *FitAoE_stab = new TF1( "FitAoE_stab","[0]*TMath::Exp( -TMath::Power( (x-[1])/[2],2. )/2. ) +[3]*((TMath::Exp([4]*(x-[5])))+[6])/((TMath::Exp((x-[5])/[7]))+[5])",fMin_stab, fMax_stab);
    
    double A_AoE_stab = h_AoE_stab->GetMaximum();
    double mu_AoE_stab = h_AoE_stab->GetBinCenter( h_AoE_stab->GetMaximumBin());
    double sigma_AoE_stab = mu_AoE_stab*0.02;

    cout << A_AoE_stab << " " << mu_AoE_stab << endl;

    FitAoE_stab->SetParameter(0, A_AoE_stab);
    FitAoE_stab->SetParLimits(0, A_AoE_stab*0.6, A_AoE_stab*1.2);
    FitAoE_stab->SetParameter(1, mu_AoE_stab);
    FitAoE_stab->SetParLimits(1, mu_AoE_stab*0.9, mu_AoE_stab*1.1);
    FitAoE_stab->SetParameter(2, sigma_AoE_stab);
    FitAoE_stab->SetParLimits(2, sigma_AoE_stab*0.2, sigma_AoE_stab*3);
    FitAoE_stab->SetParameter( 3, A_AoE_stab*0.1 );
    FitAoE_stab->SetParameter( 4, 2 );
    //FitAoE_stab->SetParLimits( 4, 0, 1);
    FitAoE_stab->SetParameter( 5, mu_AoE_stab*0.98 );
    //FitAoE_stab->SetParLimits( 5, mu_AoE_stab*0.7, mu_AoE_stab );
    FitAoE_stab->SetParameter( 6, -0.8 );
    FitAoE_stab->SetParLimits( 6, -1, 0 );
    FitAoE_stab->SetParameter( 7, 0.04);
    //FitAoE_stab->SetParLimits( 7, 0.005, 0.1 );
    FitAoE_stab->SetLineStyle(2);
    FitAoE_stab->Draw("same");
    ct->Update();
    FitAoE_stab->SetLineStyle(1);
    gStyle->SetOptFit(0);
    h_AoE_stab->Fit("FitAoE_stab","L","", fMin_stab, fMax_stab);
    TF1 *gaussian_stab = new TF1("gaussian_stab","[0]*TMath::Exp( -TMath::Power( (x-[1])/[2],2. )/2. )",fMin_stab,fMax_stab);
    gaussian_stab->SetParameters(FitAoE_stab->GetParameter(0),FitAoE_stab->GetParameter(1),FitAoE_stab->GetParameter(2));
    
    TF1 *step_stab = new TF1("step_stab", "[0]*((TMath::Exp([1]*(x-[2])))+[3])/((TMath::Exp((x-[2])/[4]))+[2])",fMin_stab, fMax_stab );
    step_stab->SetParameters(FitAoE_stab->GetParameter(3), FitAoE_stab->GetParameter(4), FitAoE_stab->GetParameter(5), FitAoE_stab->GetParameter(6), FitAoE_stab->GetParameter(7));

    gaussian_stab->SetLineColor(kAzure);
    step_stab->SetLineColor(kSpring+2);
    gaussian_stab->SetLineStyle(2);
    step_stab->SetLineStyle(2);
    gaussian_stab->Draw("same");
    step_stab->Draw("same");
    ct->Update();
    
    mu_AoE_st[i] = FitAoE_stab->GetParameter(1);
    mu_AoE_err_st[i] = FitAoE_stab->GetParameter(2);

    calib[i]=i+1;
  }
  ct->Update();
  ct->Print(Form("%s/chn%d_AoE_calib.pdf",resdir,chn));

  TCanvas *ctime = new TCanvas("ctime","ctime");
  TGraph *ht = new TGraphErrors(nCal, calib, mu_AoE_st,0,mu_AoE_err_st);
  ht->SetTitle(Form("chn%d #mu_{A/E}#pm#sigma vs calib",chn));
  ht->SetMarkerColor(4);
  ht->SetMarkerStyle(20);
  ht->SetLineColor(4);
  ht->GetXaxis()->SetTitle("calibration number");
  ht->Draw("AP");

  TF1 *cost = new TF1("cost","pol0",0.7 , 5.3);
  gStyle->SetOptFit(111);
  ht->Fit("cost","L","", 0.7, 5.3);

  ctime->Update();
  ctime->Print(Form("%s/chn%d_AoE_vs_time_s.pdf",resdir,chn));
  ...
  cout << "min_bin = " << min_bin << " max_bin = " << max_bin << " min_bin_1 = " << min_bin_1 << " max_bin_1 = " << max_bin_1 << " max_bin_2 = " << max_bin_2 << endl;
  ...
  cout << "min_bin = " << min_bin << " max_bin = " << max_bin << " min_bin_1 = " << min_bin_1 << " max_bin_1 = " << max_bin_1 << endl;
  ...
  cout << "AreaFEP_tot = " << Area_f_tot << endl;
  ...
  c->Print(Form("chn%d_DEP_FEP.pdf",chn));
  ...
  TCut cut_e3 = Form("energy[%d] > %g && energy[%d] < %g", chn, max_dep2, chn , max_dep3); 
  ...
  tier->Draw(Form("AoE[%d]/%g >>h_AoE1", chn, mean_aoe),cut_e1);
  tier->Draw(Form("AoE[%d]/%g >>h_AoE2", chn, mean_aoe),cut_e2);
  tier->Draw(Form("AoE[%d]/%g >>h_AoE3", chn, mean_aoe),cut_e3);
  ...
  TH1D *hAoE = new TH1D("hAoE",Form("chn%d;A/E;Counts",chn), nbinCut, 0.7, 1.1);
  TH1D *hAoE1 = new TH1D("hAoE1",Form(";A/E;Counts",chn), nbinCut, 0.7, 1.1);
  TH1D *hAoE2 = new TH1D("hAoE2",Form(";A/E;Counts",chn), nbinCut, 0.7, 1.1);
  TH1D *hAoE3 = new TH1D("hAoE3", Form(";A/E;Counts",chn), nbinCut, 0.7, 1.1);
  /*
  tier->Project(Form("hAoE","AoE[%d]/%g", chn, mean_aoe),cut_e);
  tier->Project(Form("hAoE1","AoE[%d]/%g", chn, mean_aoe),cut_e1);
  tier->Project(Form("hAoE2","AoE[%d]/%g", chn, mean_aoe),cut_e2);
  tier->Project(Form("hAoE3","AoE[%d]/%g", chn, mean_aoe),cut_e3);
  */

  tier->Draw(Form("AoE[%d]/%g >>hAoE", chn, mean_aoe),cut_e);
  tier->Draw(Form("AoE[%d]/%g >>hAoE1", chn, mean_aoe),cut_e1);
  tier->Draw(Form("AoE[%d]/%g >>hAoE2", chn, mean_aoe),cut_e2);
  tier->Draw(Form("AoE[%d]/%g >>hAoE3", chn, mean_aoe),cut_e3);
  ...
  double totIntegral = hAoE->Integral(hAoE->GetXaxis()->FindBin(0.), hAoE->GetXaxis()->FindBin(1.8));
  cout << "Integral = " << totIntegral << endl;
  
  double AoE_highCut = 1.+ 3.*sigma_AoE;
  double AoE_lowCut = 0.901;
  double cutIntegral;
  double SurvFrac_d;
  for (int i = 0; i < nbinCut; i++){
    cutIntegral = hAoE->Integral(hAoE->GetXaxis()->FindBin(AoE_lowCut), hAoE->GetXaxis()->FindBin(AoE_highCut));
    SurvFrac_d = (double)cutIntegral/totIntegral;
    
  if(SurvFrac_d < 0.9) break;
    else AoE_lowCut += 0.001;}
  cout << "bin = " << nbinCut << "   " << "AoE_lowcut = " << AoE_lowCut << endl;      
  ...
  legend->AddEntry(h_AoE,Form("FWHM_{A/E} = (%4.2f #pm %4.2f)%%",FWHM_AoE*100,FWHM_AoE_err*100),"l");
  ...
  TCut cut_aoe1 = Form("AoE[%d]/%g > %g && AoE[%d]/%g < %g", chn, mean_aoe, AoE_lowCut, chn, mean_aoe,AoE_highCut);
  
  tier->Draw(Form("energy[%d]>>hene_psa",chn),cut_aoe1,"sames");
  c2->Update();
  ...
  max_bin_2 =  hene->GetXaxis()->FindBin(mu_dep + 3*n_sigma*sigma_dep);
  delta = max_bin - min_bin;
  cout << "min_bin = " << min_bin << " max_bin = " << max_bin << " min_bin_1 = " << min_bin_1 << " max_bin_1 = " << max_bin_1 << " max_bin_2 = " << max_bin_2 << endl;
  ...
  for (Int_t j =  min_bin_1; j <= min_bin; j++){
    count = hene_psa->GetBinContent(j);
    sumCount += count;}
  double b2_I = sumCount;
  count = 0, sumCount = 0;
  for (Int_t j =  max_bin; j <= max_bin_1; j++){
    count = hene_psa->GetBinContent(j);
    sumCount += count;}
  double b2_II =sumCount;
  count = 0, sumCount = 0;
  for (Int_t j =  max_bin_1; j <= max_bin_2; j++){
    count = hene_psa->GetBinContent(j);
    sumCount += count;}
  double b2_III =sumCount;
  ...
  //------- Accentance and error calculation --------
  ...
  leg->Draw();
  c2->Update();
  c2->Print(Form("chn%d_PSA_DEP_FEP.pdf",chn));
  ...
  TCut cut_e = Form("energy[%d] > %g && energy[%d] < %g",chn,min,chn,max);
  ...
  hene = new TH1D("hene","; Energy [keV]; Counts",bin_in,mine,maxe);
  tier->Draw(Form("energy[%d] >>hene",chn));
  hene->GetXaxis()->SetRangeUser(min,max);
  c_4->Update();
  ...
  //----draw the A/E----    
  TCut cut_e_sep = Form("energy[%d] > %g && energy[%d] < %g",chn, mu_ - n_sigma*sigma_, chn, mu_ + n_sigma*sigma_);

  tier->Draw(Form("AoE[%d]/%g>>h_sep", chn, mean_aoe), cut_e_sep, "same");
  ...
  c_4->cd();
  TCut cut_aoe = Form("AoE[%d]/%g > %g && AoE[%d]/%g < %g", chn, mean_aoe, min_AoE_corr, chn, mean_aoe,max_AoE_corr);
  tier->Draw(Form("energy[%d] >>hene_psa",chn),cut_aoe,"sames");
  ...
  //----draw the A/E----
  TCut cut_e_fep = Form("energy[%d] > %g && energy[%d] < %g",chn, mu_ - n_sigma*sigma_, chn, mu_ + n_sigma*sigma_);

  tier->Draw(Form("AoE[%d]/%g>>h_2614", chn, mean_aoe), cut_e_fep, "same");
  ...
  cut_aoe = Form("AoE[%d]/%g > %g && AoE[%d]/%g < %g", chn, mean_aoe, min_AoE_corr, chn, mean_aoe, max_AoE_corr);
  tier->Draw(Form("energy[%d] >>hene_psa",chn),cut_aoe,"sames");
  hene_psa->GetXaxis()->SetRangeUser(min,max);
  c_5->Update();
  ...
  min = (qbb - 35);
  max = (qbb + 35);
  cut_e = Form("energy[%d] > %g && energy[%d] < %g",chn, min, chn, max);

  //----------- draw the A/E -----------
  TH1D *h_bkg = new TH1D("h_bkg","", 100, 0.7, 1.1);
  tier->Draw(Form("AoE[%d]/%g >> h_bkg", chn, mean_aoe), cut_e, "same");
